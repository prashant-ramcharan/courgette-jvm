    ...
    public boolean matches() {
        ...
        try {
            compiler.compile(cucumberFeature.getGherkinFeature()).forEach(pickle -> {
                matched.set(filters.matchesFilters(new PickleEvent(cucumberFeature.getUri().toString(), pickle)));
        ...
        try {
            pickles.forEach(pickle -> {
                ...
                if (pickleLocation != null) {
                    if (filters.matchesFilters(new PickleEvent(cucumberFeature.getUri().toString(), pickle))) {
